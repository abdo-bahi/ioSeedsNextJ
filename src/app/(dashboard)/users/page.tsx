...
function RoleBadge({ role }: { role: string }) {
  return (
    <span className={`text-[11px] px-2 py-0.5 rounded font-medium ${roleColors[role] ?? "bg-[#F5F5F5] text-[#888]"}`}>
      {role}
    </span>
  )
}
...
// ── User Modal ────────────────────────────────────────────────────
function UserModal({
  open, onClose, onSubmit, initial, isLoading, title, wilayas, isEdit,
}: {
  open:      boolean
  onClose:   () => void
      ...
      <DialogContent className="sm:max-w-[460px] max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex flex-col gap-4 py-2">
          ...
          <div className="flex items-center justify-between p-3 rounded-lg border border-[#D6E8DC] bg-[#F7F9F5]">
            <div>
              <p className="text-[13px] font-medium text-[#1A2E22]">Compte actif</p>
...
function RoleModal({
  open, onClose, userId, fields, onAssign, onRemove, existingRoles, isLoading,
}: {
  open:          boolean
  onClose:       () => void
  userId:        string
          ...
          {existingRoles.length > 0 && (
            <div className="flex flex-col gap-2">
              ...
              {existingRoles.map(rm => (
                <div key={rm.id} className="flex items-center justify-between p-2 rounded-lg bg-[#F7F9F5] border border-[#D6E8DC]">
    ...
    <div className="space-y-6">
          ...
          <table className="w-full text-[13px]">
            <thead>
              <tr className="border-b border-[#D6E8DC] bg-[#F7F9F5]">
                {["NOM", "EMAIL", "WILAYA", "RÔLES", "STATUT", "CRÉÉ LE", "ACTIONS"].map(h => (
            ...
            <tbody>
              ...
              {!isLoading && users?.map((user:any) => (
                <tr key={user.id} className="border-b border-[#F0F7F3] hover:bg-[#F7F9F5] transition-colors">
                  ...
                  <td className="px-4 py-3.5">
                    <div className="flex gap-1 flex-wrap">
                      {user.roleMembers.length > 0
                        ...
                        : <span className="text-[#8FAF9A] text-[11px]">Aucun rôle</span>
                      }
                    </div>
                  </td>
              ...
              {!isLoading && users?.length === 0 && (
                <tr>
                  <td colSpan={7} className="px-4 py-8 text-center text-[13px] text-[#8FAF9A]">
                    Aucun utilisateur. Cliquez sur &quot;Ajouter utilisateur&quot; pour commencer.
                  </td>
                </tr>
              )}
            </tbody>
      ...
      {roleUser && (
        <RoleModal
          ...
          onAssign={(role, fieldId) => assignRole.mutate({
            fk_user:            roleUser.id,
            fk_role:            role,
