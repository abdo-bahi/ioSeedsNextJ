...
function KPICard({
  ...
  return (
    <div className={`flex-1 p-4 rounded-lg border-l-4 ${color} bg-white border border-[#D6E8DC]`}>
...
function ScheduleModal({
  ...
  const { data: actuators } = trpc.actuator.getAllByField.useQuery(
    { irrigationFieldId, mcuId: selectedMcuId || undefined },
      ...
      <DialogContent className="sm:max-w-[500px] max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex flex-col gap-4 py-2">
          ...
          <div className="flex flex-col gap-1.5">
            <Label className="text-[12px] text-[#5A7A65]">Nom du programme</Label>
            <Input
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="flex flex-col gap-1.5">
              <Label className="text-[12px] text-[#5A7A65]">Heure de départ</Label>
              <Input
                type="time"
                value={form.startAt}
          ...
          <div className="flex items-center justify-between p-3 rounded-lg border border-[#D6E8DC] bg-[#F7F9F5]">
            <div>
              <p className="text-[13px] font-medium text-[#1A2E22]">Arrêt par seuil</p>
              <p className="text-[11px] text-[#8FAF9A]">Stopper si l&apos;humidité est atteinte</p>
            </div>
            <button
              onClick={() => set("toggleAtThresholds", !form.toggleAtThresholds)}
    ...
    <div className="bg-white border border-[#D6E8DC] rounded-xl p-5 flex gap-5">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 flex-wrap mb-1">
          <p className="text-[15px] font-semibold text-[#1A2E22]">{schedule.name}</p>
        ...
        <p className="text-[12px] text-[#5A7A65] mb-2">
          ...
          {triggerLabel && <> · <span className="text-[#E89B2D]">▲ {triggerLabel}</span></>}
...
export default function SchedulesPage() {
  ...
  const pausedCount   = schedules?.filter(s => !s.isActive).length ?? 0
  const actuatorCount = new Set(schedules?.map(s => s.fk_actuator)).size
  ...
  function handleCreate(form: ScheduleForm) {
    create.mutate({
      ...
      weekDays:           form.scheduleType === "weekdays"
                            ? form.weekDays as any
                            : [],
      repeatEveryDays:    form.scheduleType === "interval"
                            ? parseInt(form.repeatEveryDays)
                            : 0,
  ...
  const editSchedule = schedules?.find(s => s.id === editTarget)
    ...
    <div className="space-y-6">
      ...
      {!isLoading && (
        <div className="p-4 rounded-xl border border-[#D6E8DC] bg-white">
          <p className="text-[11px] font-semibold tracking-widest text-[#8FAF9A] uppercase mb-3">
            Configuration active — {showAll ? "Toutes les parcelles" : (selectedField?.name ?? "—")}
          </p>
          <div className="flex gap-4">
            <KPICard label="Programmes actifs"    value={activeCount}   color="border-l-[#4CAF7D]" />
            <KPICard label="Programmes en pause"  value={pausedCount}   color="border-l-[#E89B2D]" />
            <KPICard label="Actionneurs couverts" value={actuatorCount} color="border-l-[#6BA3D6]" />
          </div>
        </div>
      )}
      ...
      <div className="flex flex-col gap-4">
        {isLoading && [...Array(2)].map((_, i) => (
          <div key={i} className="h-[120px] bg-white border border-[#D6E8DC] rounded-xl animate-pulse" />
        ))}
        ...
        {!isLoading && schedules?.length === 0 && (
          <div className="bg-white border border-[#D6E8DC] rounded-xl p-8 text-center text-[13px] text-[#8FAF9A]">
            ...
            <br />Cliquez sur &quot;+ Nouveau programme&quot; pour commencer.
          </div>
        )}
      ...
      {editSchedule && (
        <ScheduleModal
          ...
          initial={{
            ...
            scheduleType:       editSchedule.weekDays?.length > 0 ? "weekdays" : "interval",
