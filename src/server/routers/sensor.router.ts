  ...
  getChartData: protectedProc
    ...
    .query(async ({ input }) => {
      ...
      const buckets = new Map<
        string,
        { sum: number; count: number; unit: null | string }
      >();
      ...
      for (const r of readings) {
        ...
        // Example:
        // 06:23 → bucket start = 06:00
        // 07:45 → bucket start = 06:00  (same bucket)
        // 09:10 → bucket start = 09:00  (next bucket)

        const bucket = new Date(
          Math.floor(ms / (bucketMinutes * 60 * 1000)) *
      ...
      return Array.from(buckets.entries()).map(([time, b]) => ({
        time,
        value: parseFloat((b.sum / b.count).toFixed(1)),
        unit: b.unit,
      }));
    }),
